...
def SPP_LS(o_id,d_id,node):
    ...
    S_ = []
    for i in node[1:]:
        if i.node_id != node[o_id].node_id:
          S_.append(i.node_id)  #初始化条件
    i = node[o_id].node_id  #i
    i1 = node[o_id]  #i和i用于存储具有最小标号的节点  #iobj
    while len(S_) != 0:  #主要循环
        least = float("inf")
        leastnum = float("inf")
        leastad = None  #三个least分别用于存储最小标号的cost，node_id和node对象地址
        for ij in i1.l_out:
# print(S," ",S_) 测试用
            j = node[ij.head_node]
            if j.u > (i1.u + ij.length):
                j.u = (i1.u + ij.length)
                j.p = i1
        for k in S_:  #关键：找出此时cost最小的点
            if node[k].u<least:
                least = node[k].u
                leastnum = node[k].node_id
                leastad = node[k]
        i1 = leastad
        i = leastnum
        S.append(leastnum)
        S_.remove(leastnum)  #将该点移出S_,放进S中
# print(S_," ",S)
# print(len(S_))  测试用
    shortestpath_p_list = [0]  # store predecessor list of each node
    for t in node[1:]:
